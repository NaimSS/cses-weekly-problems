...
OUTPUT_HTML   = Path(__file__).parent / "cses_picks.html"
...
def build_html(hard: list[dict], medium: list[dict], easy: list[dict]) -> str:
...
<head>
  ...
  <title>CSES Weekly Picks · {today}</title>
  <style>
...
def main():
    ...
    parser.add_argument("--output",    "-o", default=str(OUTPUT_HTML))
    ...
    medium = pick([p for p in available if 101 <= int(p["rank"]) <= 200], 3, rng)
    easy   = pick([p for p in available if 201 <= int(p["rank"]) <= 400], 4, rng)

    html = build_html(hard, medium, easy)
